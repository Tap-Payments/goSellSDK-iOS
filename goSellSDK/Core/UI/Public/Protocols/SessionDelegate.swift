...
@objc public protocol SessionDelegate: AnyObject, NSObjectProtocol {
    ...
    /// Notifies the receiver that apple pay successfully generated an Apple pay token
    ///
    /// - Parameters:
    ///   - token: The apple pay payment token generated by Apple pay authorization process
    ///   - session: Session object. Session instance
    @objc(appleTokenSucceed:onSession:) optional func appleTokenSucceed(_ token: PKPaymentToken, on session: SessionProtocol)
    
    
    /// Notifies the receiver that apple pay wasn't apple to generate an apple pay token.
    ///
    /// - Parameters:
    ///   - error: A descriptive message about the root cause
    ///   - session: Session object. Session instance
    @objc(appleTokenFailed:onSession:) optional func appleTokenFailed(_ error: String, on session: SessionProtocol)
